...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!code || !state) {
      return NextResponse.redirect(new URL("/register?error=missing_params", req.url));
    }
    ...
    return NextResponse.redirect(new URL("/people?registered=true", req.url));
  } catch (error) {
    console.error("Google callback error:", error);
    return NextResponse.redirect(new URL("/register?error=google_auth_failed", req.url));
  }
}
